...
# -*- coding: utf-8 -*-
"""
양양 풍력 발전량 — train_y_yangyang_norm.parquet 하나로 예측 (홀/짝수달 지원, 플랜트 자동 인식 강화 v3)
- 시간/플랜트/타깃 자동 인식 (WTGxx 류, 날짜형 숫자열은 제외)
- 홀/짝수달 필터 + 데이터 부족 시 자동 완화
- plant별 XGBoost (부족 시 글로벌 모델 백업)
- 지정일(KST) 24시간 예측 → result.csv (long)
- 옵션: result_wide.csv (pivot 기반, DataFrame fragmentation 경고 없음)
"""
...
# ====================== 사용자 설정 ======================
# 파일이 현재 스크립트와 같은 폴더에 있다고 가정
FILE = "train_y_yangyang_norm.parquet"  
PREDICT_DATE = "2023-09-16"
MONTH_FILTER = "odd"
MIN_ROWS_PLANT = 50
RANDOM_STATE = 42
SAVE_LONG = "result.csv"
SAVE_WIDE = "result_wide.csv"
PRINT_HEAD = 10
# =========================================================
...
def unify_columns(df: pd.DataFrame) -> Tuple[pd.DataFrame, str, str]:
    d = df.copy()
    nrows = len(d)

    tcol = guess_col(d, ["timestamp","end_datetime","end_dt","time","시간","datetime","date_time"])
    if not tcol:
        raise SystemExit("[에러] 시간 컬럼을 찾지 못했습니다. (예: timestamp, end_datetime 등)")
    if tcol != "timestamp":
        d = d.rename(columns={tcol: "timestamp"})
    d["timestamp"] = to_kst_aware(d["timestamp"]).dt.floor("h")

    plant_candidates = ["plant","구분","turbine_id","unit","unit_id","wtg","wtg_id"]
    pcol_guess = guess_col(d, plant_candidates)
    pcol: Optional[str] = None
    if pcol_guess and is_valid_plant_column(d[pcol_guess], nrows):
        pcol = pcol_guess

    if pcol and pcol != "plant":
    ...
    if not ycol:
        raise SystemExit("[에러] 타깃(발전량) 컬럼을 찾지 못했습니다. (예: energy_kwh, y 등)")
    if ycol != "y":
        d = d.rename(columns={ycol: "y"})

    d = d.dropna(subset=["timestamp","y"])
    d = d.groupby(["timestamp","plant"], as_index=False, observed=True).agg(y=("y","mean"))

    print(f"[INFO] 시간: {tcol} → timestamp / 플랜트: {chosen_plant_info} / 타깃: {ycol} → y")
    print(f"[INFO] rows={len(d)}, plants={d['plant'].nunique()} → {sorted(d['plant'].unique().tolist())[:10]}{' ...' if d['plant'].nunique()>10 else ''}")
...
def train_and_predict(
    ...
    dfu, _, _ = unify_columns(df)

    pred_date = pd.Timestamp(predict_date).tz_localize(KST)
    train_df = month_filter_df(dfu, month_filter, pred_date)
...
def main():
    fp = Path(FILE)
    if not fp.exists():
        raise SystemExit(f"[에러] 파일을 찾지 못했습니다: {Path(FILE).resolve()}")

    raw = pd.read_parquet(fp, engine=ENGINE)
    result = train_and_predict(raw, predict_date=PREDICT_DATE, month_filter=MONTH_FILTER)
