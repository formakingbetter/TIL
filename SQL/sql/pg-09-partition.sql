...
-- 체육대회. 1,2,3 학년 -> 한번에 1학년 순위 | 전체순위를 확인할 수 있ㄷ.
-- SELECT *, ROW_NUMBER()
...
SELECT
	...
	SUM(일매출) OVER (
		PARTITION BY DATE_TRNUNC('month',order_date)
		ORDER BY order_date
	) as 월누적매출
FROM daily_sales;
...
SELECT 
	order_date,
	일매출,
	COUNT(일매출) OVER(ORDER BY order_date),
	ROUND(AVG(일매출) OVER(
		ORDER BY order_date
	...
	ROUND(AVG(일매출) OVER(
		ORDER BY order_date
		ROWS BETWEEEN 2 PRECEDING AND CURRENT ROW
	)) AS 이동평균3일
	)
FROM daily_sales;
